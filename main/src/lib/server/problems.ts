...
export async function getExamples(problem_id: number, auth: string = ""): Promise<TestCase[]> {
    let {body} = await request("GET", `/problems/${problem_id}/test-cases?only_opened=true`, null, auth);
...
export async function createProblem(name: string, statement: string, input_statement: string, 
    ...
    let {body} = await request("POST", "/problems", {name, statement, input_statement, output_statement, 
...
export async function canBeEdited(problem_id: number, auth: string = ""): Promise<boolean> {
    let {body} = await request("GET", `/problems/${problem_id}/check-if-can-be-edited`, null, auth);
    return body.can;
}
...
export async function createTestCase(problem_id: number, input: string, solution: string, 
    ...
    let {body} = await request("POST", `/problems/${problem_id}/test-cases`, {input, solution, score, opened}, auth);
...
export async function getTastCase(problem_id: number, test_case_id: number, auth:string): Promise<TestCase> {
    let {body} = await request("GET", `/problems/${problem_id}/test-cases/${test_case_id}`, null, auth);
    return body;
}

export async function getTestCases(problem_id: number, auth: string): Promise<TestCase[]> {
    let {body} = await request("GET", `/problems/${problem_id}/test-cases`, null, auth);
    return body.test_cases;
}
